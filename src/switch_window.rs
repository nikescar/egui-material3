...
impl SwitchWindow {
    ...
    fn render_basic_switch(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label(format!("Switch state: {}", if self.basic_switch { "ON" } else { "OFF" }));
    }
    ...
    fn render_labeled_switches(&mut self, ui: &mut egui::Ui) {
        ...
        ui.vertical(|ui| {
            // Wi-Fi switch
            let mut wifi_switch = switch(&mut self.wifi_enabled)
                .text("Wi-Fi");
            if self.disabled {
            ...
            // Bluetooth switch
            let mut bluetooth_switch = switch(&mut self.bluetooth_enabled)
                .text("Bluetooth");
            if self.disabled {
            ...
            let mut notifications_switch = switch(&mut self.notifications_enabled)
                .text("Notifications");
            if self.disabled {
            ...
            // Location switch
            let mut location_switch = switch(&mut self.location_enabled)
    ...
    fn render_settings_examples(&mut self, ui: &mut egui::Ui) {
        ...
        ui.vertical(|ui| {
            ...
            let mut dark_mode_switch = switch(&mut self.dark_mode_enabled)
                .text("Dark Mode");
            if self.disabled {
            ...
            let mut auto_sync_switch = switch(&mut self.auto_sync_enabled)
                .text("Auto-sync data");
            if self.disabled {
            ...
            let mut mobile_data_switch = switch(&mut self.mobile_data)
                .text("Mobile data");
            if self.disabled {
            ...
            let mut airplane_switch = switch(&mut self.airplane_mode)
                .text("Airplane mode");
            if self.disabled {
            ...
            let mut battery_saver_switch = switch(&mut self.battery_saver)
                .text("Battery saver");
            if self.disabled {
        ...
        ui.horizontal(|ui| {
            ...
            ui.label(format!("Notifications: {}", if self.notifications_enabled { "ON" } else { "OFF" }));
        });
        ui.horizontal(|ui| {
            ui.label(format!("Location: {}", if self.location_enabled { "ON" } else { "OFF" }));
            ui.label(format!("Dark Mode: {}", if self.dark_mode_enabled { "ON" } else { "OFF" }));
            ui.label(format!("Auto-sync: {}", if self.auto_sync_enabled { "ON" } else { "OFF" }));
        });
        ui.horizontal(|ui| {
            ui.label(format!("Airplane: {}", if self.airplane_mode { "ON" } else { "OFF" }));
            ui.label(format!("Mobile Data: {}", if self.mobile_data { "ON" } else { "OFF" }));
            ui.label(format!("Battery Saver: {}", if self.battery_saver { "ON" } else { "OFF" }));
        });
        
        // Conditional warnings
        if self.airplane_mode && (self.wifi_enabled || self.bluetooth_enabled || self.mobile_data) {
            ui.add_space(8.0);
            ui.colored_label(egui::Color32::YELLOW, "⚠ Airplane mode typically disables wireless connections");
        }
