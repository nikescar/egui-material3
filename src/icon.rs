...
fn draw_home_icon(ui: &mut Ui, rect: Rect, color: Color32, filled: bool) {
    ...
    if filled {
        ...
        for i in 0..points.len() {
            ...
            ui.painter()
                .line_segment([points[i], points[next]], Stroke::new(2.0, color));
        }
    }
}
...
fn draw_search_icon(ui: &mut Ui, rect: Rect, color: Color32, filled: bool) {
    ...
    ui.painter()
        .line_segment([handle_start, handle_end], Stroke::new(3.0, color));
}
...
fn draw_settings_icon(ui: &mut Ui, rect: Rect, color: Color32, filled: bool) {
    ...
    for i in 0..8 {
        ...
        let prev_radius = if (i - 1) % 2 == 0 {
            radius
        } else {
            radius * 0.7
        };
        let prev_point = center
            + Vec2::new(
                prev_angle.cos() * prev_radius,
                prev_angle.sin() * prev_radius,
            );

        ui.painter()
            .line_segment([prev_point, point], Stroke::new(2.0, color));
    }
    ...
    if filled {
        ...
        ui.painter()
            .circle_stroke(center, radius * 0.3, Stroke::new(2.0, color));
    }
}
...
fn draw_menu_icon(ui: &mut Ui, rect: Rect, color: Color32, _filled: bool) {
    ...
    for i in 0..3 {
        ...
        ui.painter()
            .line_segment([start, end], Stroke::new(line_height, color));
    }
}
...
fn draw_check_icon(ui: &mut Ui, rect: Rect, color: Color32, _filled: bool) {
    ...
    ui.painter()
        .line_segment([start, middle], Stroke::new(2.0, color));
    ui.painter()
        .line_segment([middle, end], Stroke::new(2.0, color));
}
...
fn draw_arrow_back_icon(ui: &mut Ui, rect: Rect, color: Color32, _filled: bool) {
    ...
    ui.painter()
        .line_segment([tip, top], Stroke::new(2.0, color));
    ui.painter()
        .line_segment([tip, bottom], Stroke::new(2.0, color));
    ui.painter()
        .line_segment([tip, tail_end], Stroke::new(2.0, color));
}
...
fn draw_arrow_forward_icon(ui: &mut Ui, rect: Rect, color: Color32, _filled: bool) {
    ...
    ui.painter()
        .line_segment([tip, bottom], Stroke::new(2.0, color));
    ui.painter()
        .line_segment([tail_end, tip], Stroke::new(2.0, color));
}
...
fn draw_favorite_icon(ui: &mut Ui, rect: Rect, color: Color32, filled: bool) {
    ...
    if filled {
        ...
        // Outline heart
        ui.painter()
            .circle_stroke(left_circle, radius, Stroke::new(2.0, color));
        ui.painter()
            .circle_stroke(right_circle, radius, Stroke::new(2.0, color));
...
fn draw_notifications_icon(ui: &mut Ui, rect: Rect, color: Color32, filled: bool) {
    ...
    if filled {
        ...
        // Draw bell outline
        ui.painter()
            .line_segment([bell_top, bell_left], Stroke::new(2.0, color));
        ui.painter()
            .line_segment([bell_left, bell_bottom_left], Stroke::new(2.0, color));
        ui.painter().line_segment(
            [bell_bottom_left, bell_bottom_right],
            Stroke::new(2.0, color),
        );
        ui.painter()
            .line_segment([bell_bottom_right, bell_right], Stroke::new(2.0, color));
        ui.painter()
            .line_segment([bell_right, bell_top], Stroke::new(2.0, color));
    }
