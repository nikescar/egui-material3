...
use crate::{DrawerItem, MaterialButton, MaterialCheckbox};
...
impl DrawerWindow {
    ...
    fn render_navigation_items(&mut self, ui: &mut Ui, items: &[DrawerItem]) {
        for item in items {
            ...
            if let Some(badge) = &item.badge {
                ...
                ui.painter()
                    .circle_filled(badge_center, 12.0, get_global_color("error"));
        ...
        Window::new("Modal Drawer Demo")
            ...
            .show(ctx, |ui| {
                ...
                ui.vertical(|ui| {
                    ...
                    ui.add_space(10.0);
                    if ui
                        .add(MaterialButton::filled("Toggle Modal Drawer").small())
                        .clicked()
                    {
                        // self.is_modal_sidebar_open = !self.is_modal_sidebar_open;
                    }
                        ...
                        egui::Frame::new()
                            ...
                            .show(ui, |ui| {
                                ui.vertical(|ui| {
                                    ...
                                    if ui
                                        .add(MaterialButton::filled("Close Modal").small())
                                        .clicked()
                                    {
                                        self.is_modal_sidebar_open = false;
                                    }
        ...
        ui.push_id("drawer_controls", |ui| {
            ...
            ui.horizontal_wrapped(|ui| {
                ...
                ui.add(MaterialCheckbox::new(
                    &mut self.show_sections,
                    "Show Sections",
                ));
                ui.add(MaterialCheckbox::new(
                    &mut self.use_material3,
                    "Material 3 Theme",
                ));
            });
            ...
            ui.horizontal(|ui| {
                if ui
                    .selectable_label(
                        matches!(self.selected_drawer, DrawerType::Permanent),
                        "🔒 Permanent",
                    )
                    .clicked()
                {
                ...
                if ui
                    .selectable_label(
                        matches!(self.selected_drawer, DrawerType::Dismissible),
                        "↔️ Dismissible",
                    )
                    .clicked()
                {
                ...
                if ui
                    .selectable_label(
                        matches!(self.selected_drawer, DrawerType::Modal),
                        "📱 Modal",
                    )
                    .clicked()
                {
            ...
            ui.horizontal(|ui| {
                match self.selected_drawer {
                    DrawerType::Permanent => {
                        if ui
                            .add(MaterialButton::filled("Toggle Permanent Demo"))
                            .clicked()
                        {
                    ...
                    DrawerType::Dismissible => {
                        if ui
                            .add(MaterialButton::filled("Toggle Dismissible Demo"))
                            .clicked()
                        {
                    ...
                    DrawerType::Modal => {
                        if ui
                            .add(MaterialButton::filled("Toggle Modal Demo"))
                            .clicked()
                        {
    ...
    fn create_mail_items(&self) -> Vec<DrawerItem> {
        ...
        items_data
            .iter()
            .map(|(name, icon, badge_count)| {
                let mut item = DrawerItem::new(*name).active(self.selected_menu_item == *name);
    ...
    fn create_label_items(&self) -> Vec<DrawerItem> {
        let items_data = [("Family", "🔖", 0), ("School", "🔖", 1), ("Work", "🔖", 3)];
